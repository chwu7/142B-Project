...
Explanation method:
    ...
    Falls back to gradient-only saliency if attention weights are unavailable.
"""
import os
import re
import html
...
MODEL_PATH  = os.getenv("HF_MODEL_PATH", "experiments/hierarchical/earnings-sentiment/best_meanpool_noearlystop.pt")
...
def _token_saliency(model, input_ids, attention_mask, target_class):
    ...
    # Pass through the full aggregator is expensive for a single chunk;
    # use the CLS embedding directly with the classifier head instead.
...
def compute_saliency(model, input_ids, attention_mask, predicted_class):
    ...
    """
    chunk_scores = _chunk_attention_scores(model, input_ids, attention_mask)
...
with gr.Blocks(title="Earnings Call Sentiment Analyzer", theme=gr.themes.Soft()) as demo:
    gr.Markdown(
        ...
        *Powered by FinBERT + Hierarchical Transformer with two-level attention saliency.*
        """
    )
